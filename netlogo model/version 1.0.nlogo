...
globals [
  ...
  floating-weights; key : [type "ER"/"Office"] value : [prob mu sigma]
  nhats-attr-names
  nhats-indep-attrs
...
to load-CDs-info
  foreach csv:from-file "data\\community_district_data.csv" [row ->
...
to map-patches
  file-open "data\\patches_boroCD.csv"
...
to make-decisions
  ...
  foreach table:keys pop-ratio-each-type [this-type ->
    ...
    let floating-er-weights table:get floating-weights (list this-type "ER")
    let equal-weights-prob 1 - item 0 floating-er-weights
...
to plot-utility
  ...
  set-current-plot "Accumulative Utility Plot"
  clear-plot
  ...
  set-current-plot "Transformation of Accumulative Utility Plot"
  clear-plot
...
1956
173
Number of Each Agent Type People
Agent Type
Number of People
0.0
10.0
...
1957
805
Each Decision Type People Trending
Day
Number of People
0.0
10.0
...
"ER" 1.0 0 -7500403 true "" "plot count people with [decision-type = 1]"
"Office" 1.0 0 -2674135 true "" "plot count people with [decision-type = 2]"
"Mixture" 1.0 0 -955883 true "" "plot count people with [decision-type = 3]"
...
3003
805
Distribution of Days with In-hospital Patients
...
242
ER
count people with [decision-type = 1] / count people
17
1
14
...
242
No Choice
count people with [decision-type = 0] / count people
17
1
14
...
241
Office
count people with [decision-type = 2] / count people
17
1
14
...
240
Mixture
count people with [decision-type = 3] / count people
17
1
14
...
2481
406
Accumulative Utility Plot
Accumulative People Number
Accumulative Utility
0.0
10.0
0.0
10.0
...
2483
803
Transformation of Accumulative Utility Plot
...
1625
241
Percentage of Each Decision Type People:
14
0.0
1

@#$#@#$#@
![VNSNY](https://pbs.twimg.com/profile_images/468468834194579456/9EoqfHQf_400x400.jpeg)
# Introduction to this model
## What is this model about?
This project is a study of health condition of senior citizens in New York City which is led by Doc. Miriam Ryvicker and Professor Barry G. Silverman. This NetLogo Model simulates the decision making process between emergency room and office and clinics when seniors facing health problem based on data from Centers for Medicare and Medicaid Services (CMS) database and National Health And Trends Study (NHATS) dataset.

## Meaning of agent type
The agent people are distinguished into sixteen agent type according to four attributes:

  1. age:
    0: agent is below 75
    1: agent is 75 or above
  2. gender:
    0: agent is female
    1: agent is male
  3. race:
    0: agent is non-white
    1: agent is white
  4. dual:
    0: agent is not dual-eligible
    1: agent is dual-eligible

## How data communicate in this model?
For the most of external data especially NHATS and CMS data, they were stored in csv files located in folder "/data" under the same path. So please keep the folder under current path when running this model, otherwise it may not work. For the data generated in the running process, they were stored in some global variables and some were shown in plots for intuitive visualization. Before the decision making process proceeds, all necessary data will be loaded into the model first so that the internal data can external data could communicate normally.

## Theory of planned behavior
In the decision making process, the [theory of planned behavior](https://en.wikipedia.org/wiki/Theory_of_planned_behavior) is applied. In particular, the three terms Attitude, Subjective Norm and Perceived Behavioral Control are composed by sub-terms which come from the NHATS dataset.
### Attitude
A = w<sub>wfwt</sub> * want-find-way-to + w<sub>fc</sub> * feel-cheerful - w<sub>fb</sub> * feel-bored + w<sub>ffol</sub> * feel-full-of-life 
- w<sub>fu</sub> * feel-upset
### Subjective Norm
SN = motivation * social-norm
motivation = w<sub>keo</sub> * know-each-other + w<sub>wheo</sub> * willing-help-each-other + w<sub>cbt</sub> * can-be-trusted - w<sub>sd</sub> * self-determin + w<sub>atc</sub> * adjust-to-change - w<sub>noto</sub> * no-one-talk-to
social-norm = number-of-same-agent-type-people-making-this-decision /
 total-number-of-same-agent-type-people
### Perceived Behavioral Control
PBC = w<sub>inc</sub> * income + w<sub>edu</sub> * education

# Functionality
## Summary of functionality
With the default parameters, this model simulates the decision trend of New York City seniors from year 2008 and the result fits CMS statistical data. However, due to the randomness, the result would vary from each individual run and in some extreme cases a bad result violating the dataset may happen. In that case, re-run the model from the beginning and it is very likely a better result will happen in the second run.


## Function of each button
### Setup
Setup the environments and agents. If the envoronment and agents have already been setup, it will reset all environment and agents.
### Clear All
Clear all the environment, agents and variables.
### GO
This will run the model forever untill stopped by operator.
### Go Steps
This will run the model for several steps which is determined by input "steps"
### Overview One Community District
When this button is activated, the data of each community district can be viewed in the view window by clicking the geometry center of each community district. Clicking the blank area will close the view window and cancel selection.
### Show Heatmap
The information chosen in heatmap-info chooser will been shown in the form of heatmap. The darker color represents higher value.
Parts of the data is scraped from [NYC Planning Open Data](https://communityprofiles.planning.nyc.gov/)
(how to use the model, including a description of each of the items in the Interface tab)
### Clear Heatmap
Remove the heatmap view and reset the default map view.
### 3D Visualization
View the chosen infomation in 3D view. The first 3D view operation may take some time.

## Function of each input
### num-agents
Number of total people agents in the model
### start-year
Beginning year of this model.
### steps
Steps to run when clicking Go Steps button.
### new-seniors-rate-percentage
Increase rate of new seniors each year. For example, if there were 1,000 agents at the beginning of a year and the new-seniors-rate-percentage was set to 5, there will be 50 new senior agents coming into the model across the whole year.
### w_wfwt_er and etc.
Weights to control the significance of each sub-terms. The suffix _er or _off-cl determines whether it is used in emergency room function or office and clinics function.However, currently they are all set to 1 but may be tuned in future version.

# Function of each plots
### Number of Each Agent Type People
Number of people from each agent type.
### Each Decision Type People Trending
The y axis denotes number of people of each decision type and the x axis denotes the days. The meaning of each decision type: 

  * No Choice : people that haven't made any decisions yet.
  * ER : people that choose to go to emergency rooms for the most of times.
  * Office: people that choose to go to office and clinics for the most of times.
  * Mixture: people that sometimes choose to go to emergency rooms and sometimes choose to go to office and clinics.
@#$#@#$#@
default
true
0
