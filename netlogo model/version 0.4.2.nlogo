...
globals [
  ...
  er-cost ; key: type value: [alpha/shape beta/scale shift]
  off-cl-cost ; key: type value: [alpha/shape beta/scale shift]
  hosp-cost ; key: type value: [alpha/shape beta/scale shift]
  ;stats
...
to load-costs
  set er-cost table:make
  set off-cl-cost table:make
  set hosp-cost table:make
  file-open "data\\costs.csv"
  let headings csv:from-row file-read-line
  while [ not file-at-end? ] [
    let row csv:from-row file-read-line
    let ER-Office-Hosp item 2 row
    ifelse ER-Office-Hosp = 1
    [table:put er-cost item 4 row (list item 0 row item 1 row)]
    [ifelse ER-Office-Hosp = 2
      [table:put off-cl-cost item 4 row (list item 0 row item 1 row)]
      [table:put hosp-cost item 4 row (list item 0 row item 1 row)]
    ]
  ]
  file-close-all
end
